...
- [ ] Extend `Dim8/ModularFormsAux.lean` from the current `E4`/`E6`/`varphi`/`phi` layer to the remaining Section 4 ingredients (`h`, `psi_I`, `psi_T`, `psi_S`) and the first raw `a`/`b` profile definitions.
